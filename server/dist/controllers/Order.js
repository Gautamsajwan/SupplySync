...
const getInventoryStatusHandler = async (req, res) => {
    try {
...
const getOrdersReceivedHandler = async (req, res) => {
    try {
        const { userId } = req.params;
        const searchUser = await UserModel.findById(userId);
        if (!searchUser) {
...
const getOrdersAcceptedHandler = async (req, res) => {
    try {
        const { userId } = req.params;
        const searchUser = await UserModel.findById(userId);
        if (!searchUser) {
...
//   } catch (error) {}
// };
export { placeOrderHandler, getInventoryStatusHandler, getOrdersReceivedHandler, getOrdersAcceptedHandler, };
